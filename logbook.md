# Logbook

| Date  | Used hours | Subject(s) |  Outcome(s) |
| :---         |     :---:      |     :---:      |     :---:      |
| 7.1.2026 | 5 | Completing the Cisco course content and participated in the lecture | Completed 'My Knowledge Check', 'Module 1' and 'Module 2' sans the quizes (they weren't available). |
| 8.1.2026 | 3 | Completing the Cisco course content | Completed quizzes for 'Module 1', 'Module 2' and completed 'Module 3' including the quiz|
| 9.1.2026 | 3 | Completing the Cisco course content | Completed modules 4 and 5 (including quizes), filled out the end of course survey, and completed the Course Final Exam|
| 15.1.2026 | 2 | Participating in the lecture | Participated in the lecture|
| 21.1.2026 | 4 | Reading through course content on Portswigger.net and participating in the lecture | Studied SQL injection methods, completed the first lab and learned about next task on lecture |
| 23.1.2026 | 4 | Completing labs on Portswigger.net | Did all the required labs for the assignment and wrote reflections about all of them |
| 27.1.2026 | 4 | Setting up testing environment for ZAP scan | Completed ZAP scan and made ZAP report |
| 3.2.2026 | 2 | Writing the test report | Due to technical issues, got the test report only halfway done |
| 4.2.2026 | 2 | Finished the test report | Finalized the test report, ran additional test with Burp and wrestled with github |
| 11.2.2026 | 2 | Working on part2 of the booking system assignment | Finished the assignment and submitted |
| 16.2.2026 | 4 | Working on phase2 assignment | Made a ZAP report, used hashcat to crack passwords and submitted the assignement |
| 1.3.2026 | 2 | Working on phase3 assignment | Made a ZAP report, completed browser testing and auth testing |
| 11.3.2026 | 1,5 | Working on phase4 assignment | Went through GDPR checklist and added requested files to repo |
| 18.3.2026 | 2 | Working on portswigger labs | Completed SQL injection labs |
| 22.3.2026 | 3 | Working on portswigger labs and the final report for the course | Completed Cross-site scripting labs and wrote the final report for the course |
